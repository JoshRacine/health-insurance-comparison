...
exports.calculatePlanCost = calculatePlanCost;
exports.comparePlans = comparePlans;
function calculatePlanCost(plan, usage, isFamily, monthlyPremium, employerHSAContribution = 0) {
    ...
    const specialtyDrugCostPerMonth = Math.min(avgSpecialtyDrugCost * plan.specialtyDrugCoinsurance, plan.specialtyDrugMaxCopay);
...
function comparePlans(plans, usage, isFamily, premiums, hsaContributions) {
    const results = {};
    for (const [key, plan] of Object.entries(plans)) {
        results[key] = calculatePlanCost(plan, usage, isFamily, premiums[key] || 0, hsaContributions[key] || 0);
    }
    return results;
}
