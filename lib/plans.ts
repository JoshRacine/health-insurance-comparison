export interface PlanData {
  ...
  specialtyDrugCoinsurance: number; // 0.30 = 30%
  specialtyDrugMaxCopay: number; // $250 cap
  // Additional fields for premium and HSA
  monthlyPremiumIndividual?: number;
...
export interface UsageInputs {
  ...
  // Prescriptions (monthly average)
  genericDrugsPerMonth: number;
  preferredBrandDrugsPerMonth: number;
  nonPreferredBrandDrugsPerMonth: number;
  specialtyDrugsPerMonth: number; // NEW

  // Other costs
  labTestsXrays: number; // Cost estimate
  imagingCTPETMRI: number; // Cost estimate

  // NEW: Easy to add services
  mentalHealthTherapySessions: number; // Outpatient therapy
  outpatientSurgeries: number; // Ambulatory surgery
  physicalTherapySessions: number; // PT/OT visits

  // Family configuration
  familyMembers: number;

  // NEW: Maternity planning
  planningPregnancy: boolean;
}

export const defaultUsageInputs: UsageInputs = {
  primaryCareVisits: 4,
  specialistVisits: 2,
